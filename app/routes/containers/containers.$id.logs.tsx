...
export default function ContainerIdLogs({ params }: Route.ComponentProps) {
  ...
  const { data: keywords = [] } = useQueryStatus<string[]>(["keywords"]);
  ...
  useEffect(() => {
    ...
    const filteredResults = logs.filter((entry) => {
      ...
      const matchesText =
        ...
        entry.timeStamp.includes(filter.text);
          ...
          {filteredLogs.length > 0 ? (
            filteredLogs.map((log, index) => {
              return (
                ...
                >
                  {/* <span className="font-medium text-muted-text">{index + 1}</span>
                  <span className="mx-4xs">-</span> */}
                  ...
                  <span
                    className={clsx(keywords.some((keyword) => log.message.includes(keyword)) ? "text-error-400 light:text-error-500" : "text-text")}
                  >
                    {log.message}
                  </span>
                </p>
